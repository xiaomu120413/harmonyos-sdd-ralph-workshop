...
> CPU/软件路径、未走 GPU 是用户已冻结的案例背景。本视频的任务是把该背景转换为可复核的同 run 原始证据；未录制前保留 `PENDING`。

- 20–30 秒，固定设备、服务端、网络、分辨率和同一视频片段。
- 画面同时出现远端视频、路径 diagnostics、CPU/FPS 或性能采样。
- 开头先展示 `software/OpenH264/FFmpeg fallback` 等真实路径证据；如果路径日志采集失败，仍可保留卡顿视频，但证据表中的 path 项必须继续标 `PENDING`。
