...
播放 10 秒卡顿录屏，先收集观察，不允许回答“上硬解”“改 GPU”或“加线程”。
...
- GDI background 只参与 AVC420 的可信 base / retained 合成。
...
```text
frameId path event pts queueDepth durationMs owner result reason
```
...
从一段真实日志中过滤一个 frameId，按时间排序，标出最早异常；保存为 `gpu-frame-trace.txt`。
...
给两组样例：A `decode=3ms, queueAge=95ms`；B 各段都低但首个 decoded sample 全白。让学员分别写下一条证伪日志。
...
用 `rg` 搜三套 decoder，分别标注调用入口、输出对象和最终 presenter；让 AI 解释本轮修改为什么只属于其中一套。
...
依次输入：full update、partial+fresh snapshot、partial+no snapshot；观察 owner 是否只在前两种接管。
...
- Active 队列满 720 时 enqueue 失败，当前 command 转同步处理；可能越过旧队列工作，属于顺序 GAP。
...
4. Surface generation 变化后，旧 callback 不能写新 target。
...
每组任选一个 Case，写出：最早异常、下一轮唯一修改边界、反证、需要保存的同 frameId 日志。
