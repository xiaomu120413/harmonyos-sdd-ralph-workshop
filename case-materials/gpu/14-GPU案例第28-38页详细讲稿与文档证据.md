...
页面左侧展示 `17-复杂三方项目GPU适配源码分析与探针方案.md` 的方案推导与十步路径，右侧展示 `18-GPU适配实施任务包与验收点.md` 的任务入口。先说明“为什么这样改”，再说明“下一轮 AI 具体改什么”：
...
PPT 不只放十步摘要：必须保留 `path::symbol`、输入证据、candidate gate、original fallback 和 matched EndFrame 五类锚点，让观众看见步骤是由源码约束推导出来的，不是 AI 凭空列出的开发清单。

---

## 第 36 页｜迭代开发：每轮只增加一个已验证能力

页面直接截取 `18-GPU适配实施任务包与验收点.md`。每轮都按 Ralph Worker Packet 执行：
...
讲师重点不是念 I0–I6，而是展示 G3/G4/G6 三张任务卡：G3 证明 Bridge 可逆，G4 证明硬解 output 与 NativeBuffer，G6 证明 matched EndFrame、owner 与 fallback。任何一张证据不闭合，都不得把后续任务发给 AI。
...
| 34 | 同文档第 6 节最终方案 |
| 35 | `17-复杂三方项目GPU适配源码分析与探针方案.md` 第 7–12 节；`18-GPU适配实施任务包与验收点.md` 固定输入与全局约束 |
| 36 | `18-GPU适配实施任务包与验收点.md` G0–G8 Story、AC、Stop 与证据模板 |
