...
文中的行号用于课堂快速定位，后续提交可能导致行号变化；评审时以“文件路径 + 函数名”为主，以 commit 为复现基线。
...
## 1. 课堂从哪里切入：先追一条命令，不先读完整仓库

### 1.1 先把需求改写成可追踪的问题

原始需求不是“实现 HarmonyOS 硬解”，而是：

> 用户报告远控视频播放卡顿，并怀疑 CPU/软件解码路径。希望先确认真实 consumer/decoder path，再评估 HarmonyOS 硬件解码与 GPU 合成，同时保留可解释回退。当前性能根因与改善幅度均未证实。
...
任务不是按“模块数量”平均切，而是按**可独立验证的风险闭环**切。每张任务卡必须只有一个主要结果，并明确允许修改的源码边界、禁止扩散的范围和 stop 条件。
...
1. **先让学员判断。** 只给“CPU 高、视频卡”的现象，问他们会让 AI 先改哪里。
2. **现场追符号。** 从 `gfx->SurfaceCommand` 追到 wrapper，再揭示 original callback 被保存。
3. **在 consumed 门暂停。** 让学员判断哪些条件满足后才允许 suppress GDI，再展示源码顺序。
4. **给出黑屏假设。** 告诉大家 decoder 已有 output，让他们在调用图上选下一处探针。
5. **揭示 EndFrame。** 对比“decode 后立即 swap”和“pending 到 matched EndFrame”的差异。
6. **让学员先拆任务。** 再用源码状态所有权解释为什么 T02、T03、T04 不能合成一个“大任务”。
7. **最后才看结果。** 用四层证据矩阵判断哪些是 PASS、哪些仍是 UNKNOWN。

这套讲法的重点不是展示 AI 写了多少代码，而是让学员看到：AI 给出的每个结论如何被源码证据约束，错误时如何沿调用链被推翻，复杂需求如何被拆成可停止、可复查、可验收的工程闭环。
