...
→ FirewallLocalRepository / ModeSwitch / PolicyService
...
| 字段 | 所有者 | 什么时候更新 |
|---|---|---|
| `previousUserIds` | Coordinator | 取得稳定快照后 |
| `desiredEnabled` | LocalRepository | 总开关成功后 |
| `lastAppliedMode` | LocalRepository | 模式成功应用后 |
| `lastAppliedUserIdsSignature` | LocalRepository | 最新集合处理成功后 |
| intents/deployments | LocalRepository | 自定义规则事务成功后 |
...
- 现场可点开 `AccountChangeCoordinator.ets`、`FirewallAccountChangeHandler.ets`、`SystemUserProvider.ets` 三个真实文件，分别指出“协调、业务、真相源”。
- 学员应该能看懂为什么 `SystemUserProvider` 不直接做策略下发。
...
- 不逐个讲 Diff；重点讲“每轮为什么没有完成整个 Feature，以及下一轮如何由证据触发”。
...
1. `Case JSON` 描述业务步骤和断言，不绑定具体设备工具。
2. `SecurityToolAdapter + Flow Executor` 把业务动作路由到 MCP Driver 或 HDC Driver。
3. `Assertions` 判定结果，`Reporters` 同时保存 Results 与 Artifacts。

### 现场实操

- 输入：`scripts/e2e/cases/firewall/status_toggle.json` 或 `mode_cards.json`。
- 学员动作：在图上标出 `launch_app`、`open_firewall_page`、`toggle_firewall`、`assert_text_presence` 分别经过哪条路径。
...
- 图片尺寸约 1873×382，适合横向铺满页面中部；不要裁掉右侧 `HarmonyOS MCP Tools`，也不要纵向拉伸。
- 这页讲架构与路由，不重复讲 MCP 产品能力。
- 如果现场没有设备，仍然可以完成 Case JSON 到 Driver/Assertion 的静态走读。

### 文档 / 截图

- 用户图片：`C:/Users/mu/AppData/Local/Temp/codex-clipboard-6778939b-3ce1-44ae-baf5-06a00b13fb43.jpg`
- 真实代码：SecurityTool `scripts/e2e/core/`、`adapters/security_tool/`、`drivers/`、`reporters/`。
- `PLACEHOLDER`：后续将临时图片复制到正式媒体目录，避免 Temp 路径失效。
