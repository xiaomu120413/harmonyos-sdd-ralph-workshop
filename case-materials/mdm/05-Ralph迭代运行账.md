...
> **迭代结论（CURRENT）：** 迭代的价值不是“AI 多跑几轮”，而是每个反例都被写回设计、测试和下一轮的约束。
...
**运行账必须保留的链条：** 症状 → 根因 → 新不变量 → 下一轮 oracle。只有 commit 列表不能说明为什么 AI 修对了。
...
- 系统/实物：**[需用户补充 U01–U08]** 完整矩阵仍为 PENDING；工程开放项 E01/E02 不计入用户责任。
...
本轮结论是：“功能可用”不等于“需求完成”，因为验收标准还要求资产可管理。
