...
```mermaid
sequenceDiagram
  participant H as 需求负责人
  participant A as AI执行会话
  participant D as 设计文档
  participant C as 源码和测试
  participant G as Git和历史会话
...
资产同步的输出不能只是目录树；上述五份文档必须把“读过代码”转换成下一阶段可直接消费的结构化输入。
...
- 决策：明确区分“业务身份粒度”和“系统执行粒度”，验收必须插入同类型第二台设备验证影响面。
...
- 决策：代码、2.1 权威规格和 2026-07-11 Story 互相印证，旧句标记为文档债务，不进入当前实施结论。
...
> **决策结论（CURRENT）：** 资产同步不是“读了多少文件”，而是找出会改变架构和验收方式的冲突。
...
**该摘要可以证明：** 为什么要先同步资产、记录冲突，再让 AI 写方案。

**该摘要不能证明：** 设备策略已在真机完整通过；真机结论仍需系统回读和实物证据。
...
- [x] 每个强结论标注证据级别。
- [ ] **[需用户补充 U01–U08]** 全局/默认/显式策略完整实物矩阵；拍摄要求见 `16-用户补充素材清单.md`。
