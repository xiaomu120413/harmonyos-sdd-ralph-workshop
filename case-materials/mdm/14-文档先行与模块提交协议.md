...
这份协议解决一个工程化问题：AI 已经能写代码以后，如何让多轮 Session、多模块并行和后续维护仍然可追踪，而不是把关键决策留在聊天记录里。
...
协议只定义记录格式；MDM 外设案例 S1–S8 的完整实现规格见 `04-Feature与Story拆解.md`，历史实现锚点、提交身份与当前证据状态见 `15-模块变更账与Session交接.md`。

两份文档分工如下：

- 本文回答“以后所有模块都必须怎么提交和记录”。
- `15-模块变更账与Session交接.md` 回答“这些 Story 的文档、代码、测试和证据当前绑定到什么程度”。
- `05-Ralph迭代运行账.md` 记录每轮假设、RED、验证和下一个未知。
- `06-测试验收报告.md` 给出当前可判定结果，不把 Ledger 中的 `PENDING` 汇总成 PASS。
