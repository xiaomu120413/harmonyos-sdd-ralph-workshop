...
课堂口径要特别说明：“接口管控启用”表示所有设备通过全局闸门，并不等于给每台设备写一条白名单记录。通过全局闸门后，已有设备仍受单设备显式规则约束；只有没有显式规则的新设备才读取默认黑/白名单。
...
sequenceDiagram
  ...
  participant VM as Policy ViewModel
  participant S as State Service
  participant M as MDM usbManager
  participant R as Policy State RDB
  participant T as Trace RDB

  U->>V: 选择“禁止接入”
  V->>VM: setDevicePolicy(fingerprint, deny)
  VM->>S: 校验 global / present / storage
  ...
  alt MDM 失败
    ...
    M-->>S: success
    S->>R: upsert desired=deny, active=deny
...
sequenceDiagram
  actor U as 管理员
  participant VM as Interface VM
  participant G as Global Service
  participant S as Device State Service
  participant R as restrictions usb
  participant P as Policy RDB

  U->>VM: 全局禁止 USB
  VM->>G: setDisabled(true)
  G->>S: suspendActivePolicies()
  S->>P: 保留 desired，active→none
  G->>R: setDisallowedPolicy(usb,true)
  alt 全局下发失败
    R-->>G: error
    G->>S: compensate / restore present deny
    S->>P: active→deny
    G-->>VM: FAIL，UI 保持全局旧值
  else 全局下发成功
    R-->>G: success
