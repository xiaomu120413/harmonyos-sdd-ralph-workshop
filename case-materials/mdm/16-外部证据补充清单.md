...
工程文档中若写 `PENDING`，必须同时指向一个 `Uxx` 或 `Exx`；这样能区分“等待外部证据”还是“工程自身还没完成”。
...
第一轮优先采集 U01–U04，补齐“全局事务、首次接入、系统残留、部分生效”四组关键证据。U05–U08 作为扩展验证，不阻塞第一轮工程复核。
...
| L05 | MCP/E2E 架构示意图、证据阶梯和 Change Ledger 图 | 当前工程文档，可重新生成 |
...
文档遇到 E01–E03 时标记 **`[工程开放项]`**，不要标记成 **`[需外部补充]`**。
