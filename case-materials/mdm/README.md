...
> 证据标记统一使用：`[需外部补充 Uxx]`、`[工程开放项 Exx]`、`[可本地生成 Lxx]`。完整清单见 [16-外部证据补充清单.md](16-外部证据补充清单.md)，不要把所有 `PENDING` 都理解为需要外部提供。

这个案例不是“让 AI 写一个 USB 开关”，而是把一个存在多层策略、系统真源、失败补偿和动态状态的复杂需求，收敛成可以开发、循环和验收的工程交付。入口是一个真实现象：页面只有几个选择器，但一次“允许/禁止”会同时穿过页面状态、设备身份、应用规则、EDM/MDM 类型策略、系统重新枚举和真实硬件行为。
...
| 源码穿刺 | [08-代码级调用链与验证穿刺.md](08-代码级调用链与验证穿刺.md) | 从 UI 一路追踪到 MDM 和状态库 |
| Session 证据卡 | [13-本地Session真实问题证据卡.md](13-本地Session真实问题证据卡.md) | 每个真实问题的现象、调用链、证据与截图位 |
| 文档与提交协议 | [14-文档先行与模块提交协议.md](14-文档先行与模块提交协议.md) | 如何先改文档、按模块提交、记录 Change Ledger，并组织 AI 易读代码 |
| 模块变更账 | [15-模块变更账与Session交接.md](15-模块变更账与Session交接.md) | doc/code/test/accepted commit、运行证据、开放项和新 Session 交接 |
| 外部证据清单 | [16-外部证据补充清单.md](16-外部证据补充清单.md) | 哪些必须外部采集、怎么命名，以及哪些其实是工程开放项 |
...
- **[需用户补充 U01–U08]** 完整 USB 实物矩阵尚未形成证据包，相关页面必须保留 `PENDING` 视频与系统回读位。
