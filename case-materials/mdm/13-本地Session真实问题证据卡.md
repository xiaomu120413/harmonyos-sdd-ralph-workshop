...
| 编号 | 建议素材 | 当前 |
|---|---|---|
| S-A | 旧默认策略入口与当前 restrictions 调用对比 | 可从源码截图 |
| S-B | 默认 allow 名单为空 → 修复后白名单卡片 | `补充素材` |
| S-C | EDM `baseClass=3` 回读或数据库查询 | `补充素材` |
| S-D | `9200010` UI + 系统残留对照 | `补充素材` |
| S-E | `9200007` hilog 关键四行 | 可从 Session/日志重建，需脱敏 |
| S-F | Trace RDB 时间线 + 缺失刷新日志 | 可画时序图 |
| S-G | 离线卡片与 `getDevices()` 对账 | `补充素材` |
| S-H | 全局恢复后显式 deny 重放 | 可画时序图，真机视频 PENDING |
